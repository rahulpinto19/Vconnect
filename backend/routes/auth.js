...
router.post("/sendotpreg", async(req, res) => {
  ...
  if (userExist) {
    ...
    else 
    {
        ...
        const newuserverification = new userverification({name:name,email:email,otp:otp});
        await newuserverification.save().then(()=>
        {
            var mailOptions = {
                from: "rahulyadav252424@.com",
                to: email,
...
router.post('/signup',async(req,res)=>
{
    ...
    if(Userverification)
    {
        ...
        {
            const newUser = new user({name:name,email:email,otp:otp,password:password});
            newUser.save().then(()=>
            {
